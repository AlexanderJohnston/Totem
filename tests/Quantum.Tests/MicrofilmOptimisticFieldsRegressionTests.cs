...
namespace Quantum.Tests
{
  ...
  public class LegacyMicrofilmOptimisticFieldsRegressionTests : TopicTests<MicrofilmTableTopic>
  {
    static readonly Id ClientId = Id.From("00000000-0000-0000-0000-000000000101");

    [Fact]
    public async Task LegacyClientCreateAndUpdate_AcceptUnknownArbitraryScalarFieldsWithoutDefaults()
    {
      await Append(new ClientCreated(new KnownClient("Job", "JOB-001", ClientId, Id.Unassigned)));
      await Append(new CreateMicrofilmRegularRow(ClientId, "legacy-1", new Dictionary<string, MicrofilmCellValue>
      {
        ["note"] = MicrofilmCellValue.FromText("legacy"),
        ["count"] = MicrofilmCellValue.FromNumber(4),
        ["flag"] = MicrofilmCellValue.FromCheckbox(false),
        ["emptyValue"] = null
      }));
      var created = await Expect<MicrofilmRegularRowCreated>();

      await Append(new UpdateMicrofilmRegularRowCell(ClientId, "legacy-1", " unknownAfterCreate ", MicrofilmCellValue.FromCheckbox(true)));
      var updated = await Expect<MicrofilmRegularRowCellUpdated>();

      Assert.Equal("legacy", created.Row.Cells["note"].Text);
      Assert.Equal(4, created.Row.Cells["count"].Number);
      Assert.False(created.Row.Cells["flag"].Checkbox);
      Assert.Equal(MicrofilmCellValueKind.Null, created.Row.Cells["emptyValue"].Kind);
      Assert.DoesNotContain("boxName", created.Row.Cells.Keys);
      Assert.DoesNotContain("rollName", created.Row.Cells.Keys);
      Assert.True(updated.Row.Cells["unknownAfterCreate"].Checkbox);
    }

    [Theory]
    [InlineData("")]
    [InlineData(" ")]
    public async Task LegacyClientCreate_RejectsEmptyFieldIds(string columnId)
    {
      await Append(new ClientCreated(new KnownClient("Job", "JOB-001", ClientId, Id.Unassigned)));
      await Append(new CreateMicrofilmRegularRow(ClientId, "legacy-1",
        new Dictionary<string, MicrofilmCellValue> { [columnId] = MicrofilmCellValue.FromText("rejected") }));
    ...
    [InlineData("{}")]
    [InlineData("[]")]
    public async Task LegacyClientCreate_RejectsObjectAndArrayCellRepresentations(string json)
    {
      await Append(new ClientCreated(new KnownClient("Job", "JOB-001", ClientId, Id.Unassigned)));
      var value = JsonSerializer.Deserialize<MicrofilmCellValue>(json);
      Assert.Equal(MicrofilmCellValueKind.Unsupported, value.Kind);
      ...
      await Append(new CreateMicrofilmRegularRow(ClientId, "legacy-1",
        new Dictionary<string, MicrofilmCellValue> { ["metadata"] = value }));
    ...
    [InlineData("{}")]
    [InlineData("[]")]
    public async Task LegacyClientUpdate_RejectsObjectAndArrayCellRepresentations(string json)
    {
      await Append(new ClientCreated(new KnownClient("Job", "JOB-001", ClientId, Id.Unassigned)));
      await Append(new CreateMicrofilmRegularRow(ClientId, "legacy-1", new Dictionary<string, MicrofilmCellValue>()));
      await Expect<MicrofilmRegularRowCreated>();
      var value = JsonSerializer.Deserialize<MicrofilmCellValue>(json);
      Assert.Equal(MicrofilmCellValueKind.Unsupported, value.Kind);

      await Append(new UpdateMicrofilmRegularRowCell(ClientId, "legacy-1", "metadata", value));

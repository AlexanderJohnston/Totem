...
public sealed class EventInfo : MessageInfo
{
    ...
    public static EventInfo From(Type type)
    {
        if(!TryFrom(type, out var info))
            throw new ArgumentException($"Expected type to be a public, non-abstract class implementing {typeof(IEvent)}", nameof(type));

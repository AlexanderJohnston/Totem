...
internal sealed class TopicWhenMethodParameterBuilder : RuntimeMethodParameterBuilder<CommandParameter>
{
    internal TopicWhenMethodParameterBuilder(ParameterInfo parameter) : base(parameter)
    { }
    ...
    internal override void Reflect(RuntimeMapBuilder map)
    {
        ...
        var commandType = isCommand ? type :
            type.GetImplementedInterfaceGenericArguments(typeof(ICommandContext<>)).SingleOrDefault()
            ?? type.GetImplementedInterfaceGenericArguments(typeof(ITopicContext<>)).SingleOrDefault();

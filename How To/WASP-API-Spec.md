...
| `POST` | `/public-api/customers/GetCustomersByNumber` | `WaspResult<List<WaspResult<CustomerInfo>>>` | `CustomerController.GetByNumberAsync()` |
...
These routes currently cover the first-pass WASP façade used by the Quasar frontend, including the Explorer searches and customer number lookup.

`POST /public-api/customers/GetCustomersByNumber` expects a raw JSON array of customer number strings and forwards that exact array unchanged to WASP, for example:

```json
["202541823"]
```

It preserves the nested WASP response envelope for each requested customer record instead of flattening the inner `Data` object.
...
|----------|------|-------|
| `RowNumber` | `int` | |
| `ImportRowNumber` | `int` | Present on lookup-style customer responses |
| `CustomerId` | `int?` | *Read-only* |
| `CustomerNumber` | `string` | Unique identifier |
| `CustomerName` | `string` | |
| `CustomerDepartment` | `string` | Present on richer customer lookup responses |
| `CustomerCompanyName` | `string` | Present on richer customer lookup responses |
| `FirstName` | `string` | |
| `LastName` | `string` | |
| `Email` | `string` | |
| `Department` | `string` | |
...
| `Addresses` | `List<AddressInfo>` | Returned by customer number lookup responses |
| `CustomFields` | `List<DcfValueInfo>` | |
| `HasAttachment` | `bool` | |
| `IsAutogeneratedCustomerNumber` | `bool` | |
| `AttachmentsToAdd` | `List<string>` | |
| `AttachmentsToDelete` | `List<string>` | |
| `AttachmentNames` | `List<KeyValuePair<string, string>>` | |
...
| **GetByNumberAsync** | `(IReadOnlyList<string>) → WaspResult<List<WaspResult<CustomerInfo>>>` | Get by numbers with per-record WASP envelopes |

### `GetByNumberAsync` response shape

Unlike `AdvancedSearchAsync`, `GetByNumberAsync` preserves the per-record WASP envelope:
...
```json
{
  "Data": [
    {
      "Data": {
        "CustomerNumber": "202112418",
        "CustomerDepartment": "Film & Fiche",
        "CustomerCompanyName": "Historical Society of Carroll County",
        "Addresses": [],
        "CustomFields": [],
        "HasAttachment": false
      },
